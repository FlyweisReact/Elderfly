...
const Subs = () => {
  ...
  const deleteSubscription = async (id) => {
    try {
      const { data } = await axios.delete(`https://sqd1nkfeu1.execute-api.ap-south-1.amazonaws.com/development/api/v1/subcriptions/${id}`)
      console.log(data)
              ...
              <div className="subes-box" key={index}>
                <div className="sub-box-item">
                  ...
                  <div className="sub-box-item-below">
                    <button>Edit </button>
                    <button onClick={() => deleteSubscription(i._id)}>Remove </button>
                  </div>
                </div>
              </div>
            ))}
