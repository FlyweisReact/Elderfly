...
const Main = () => {
  ...
  const Login = async () => {
    try {
        const { data } = await axios.post("https://sqd1nkfeu1.execute-api.ap-south-1.amazonaws.com/development/api/v1/auth/login" , {
            email  , password
        })
        ...
        <div className="main-cont-right">
          <div className="main-cont-right-1">
            ...
            <form className="form-main" onSubmit={Login}>
              <input className="ip" type="text" placeholder="Admin Id" onChange={(e) => setEmail(e.target.value)} />
