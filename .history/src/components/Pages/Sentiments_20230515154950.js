...
import HOC from '../HOC'import { Table } from "react-bootstrap";
...
const Sentiments = () => {
        ...
        >
          ...
          <Modal.Body>
            <Form onSubmit={postHandler}>
              ...
              <Form.Select aria-label="Default select example" className="mb-3" onChange={(e) => setSubService(e.target.value)} > 
                <option>-- Select Sub-Service --</option>
                {subData?.map((i , index) => (
                  <option key={index} value={i._id}> {i.subServices} </option>
                ))}
              </Form.Select>
    ...
    const deleteHandler  = async (id) => {
      try{
          const { data } = await axios.delete(`https://sqd1nkfeu1.execute-api.ap-south-1.amazonaws.com/development/api/v1/servic/${id}`)
            ...
            <tbody>
              {data?.map((i, index) => (
                <tr key={index}>
                  ...
                  <td>
                    <span>
                      <i className="fa-solid fa-trash" onClick={() => deleteHandler(i._id)}></i>
                    </span>
                  </td>
                </tr>
              ))}
