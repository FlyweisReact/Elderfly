...
const ConciergService = () => {
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const fetchServices = async () => {
      try { 
        const { data }  = await axios.get("https://nishant-jain12.vercel.app/api/v1/servic")
      ...
      >
        ...
        <Modal.Body>
          <Form onSubmit={postHandler}>
            ...
            <Form.Select aria-label="Default select example" className="mb-3"   onChange={(e) => setTerms(e.target.value)}>
      <option>Select Services</option>
      {services?.map((i , index) => (
        <option key={index} value={i._id} > {i.service} </option>
      ))}
    </Form.Select>
